...
export class SaveWorkflowSchemaUseCase {
  ...
  async execute(port: SaveWorkflowSchemaUseCasePort) {
    const exisitingExecution =
      await this.workflowExecutionRepository.getWorkflowExecutionsBySchemaId(
        port.id,
      );
    if (exisitingExecution) {
      const schema = await this.workflowSchemaRepository.get(
        exisitingExecution.workflowSchemaId,
      );
    ...
    return this.workflowSchemaRepository.update(port.id, {
      schema: port.schema,
    });
  }
}
