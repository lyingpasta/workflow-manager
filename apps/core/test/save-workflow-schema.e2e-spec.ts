  ...
  describe("when schema exists", () => {
    describe("when no execution exists yet", () => {
      it("should save the new schema", async () => {
        ...
        const updatedSchema = await prismaService.workflowSchema.findUniqueOrThrow({ where: { id: workflowSchemaId } })
        expect(updatedSchema).toMatchObject(expect.objectContaining({
          schema: { test: 'value' }
        }))
      })
    })
    ...
    describe("when already execution exists", () => {
      it("should create a new schema", async () => {
        ...
        const existingSchemas = await prismaService.workflowSchema.findMany({ where: { workflowId: workflow.id } })
